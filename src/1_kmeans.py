...
if __name__ == "__main__":
    t0 = time()
    parser = argparse.ArgumentParser(description='create kmeans features',
    ...
    ## PROCESS
    cls = KMeans(n_clusters=in_clusters)
    KMeans_train = cls.fit_transform(train)
    KMeans_test = cls.transform(test)
    
    ## SAVE
    np.savetxt(FFOLD+'kmeans_'+str(in_clusters)+'_'+iinput+'.train.csv',KMeans_train)
    np.savetxt(FFOLD+'kmeans_'+str(in_clusters)+'_'+iinput+'.test.csv',KMeans_test)
