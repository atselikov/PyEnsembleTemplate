...
if __name__ == "__main__":
    ...
    ## PROCESS
    cls = GaussianRandomProjection(n_components=in_clusters, random_state=RANDOM_STATE)
    cls_train = cls.fit_transform(train)
    cls_test = cls.transform(test)
    
    ## SAVE
    np.savetxt(FFOLD+'grp_'+str(in_clusters)+'_'+iinput+'.train.csv',cls_train)
    np.savetxt(FFOLD+'grp_'+str(in_clusters)+'_'+iinput+'.test.csv',cls_test)
