...
if __name__ == "__main__":
    ...
    parser.add_argument('alg_id', type=int, help='xgb_id')
    ...
    parser.add_argument('max_features', type=float, help='max_features')
    parser.add_argument('scaled', type=float, help='scaled')
    ...
    for i, (train_index, valid_index) in enumerate(SKFOLDS):
        ...
        for bag in range(BAGS):
            ...
            clf = GradientBoostingRegressor(n_estimators=args.est, max_depth = args.max_depth, max_features=args.max_features, random_state=rs)
    ...
    ## SAVE
    np.savetxt(L1FOLD+str((100000*cv_results.mean()).astype(int))+'_'+'gbr_1l_param'+str(j)+'.train.csv',blend_train)
    np.savetxt(L1FOLD+str((100000*cv_results.mean()).astype(int))+'_'+'gbr_1l_param'+str(j)+'.test.csv',blend_test)
