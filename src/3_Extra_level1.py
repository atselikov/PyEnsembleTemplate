...
if __name__ == "__main__":
    ...
    for i, (train_index, valid_index) in enumerate(SKFOLDS):
        ...
        for bag in range(BAGS):
            ...
            clf = ExtraTreesRegressor(n_estimators=args.est, max_features=args.max_f, max_depth =args.max_d, random_state=rs, n_jobs=-1)
    ...
    ## SAVE
    np.savetxt(L1FOLD+str((100000*cv_results.mean()).astype(int))+'_'+'extra_1l_param'+str(j)+'.train.csv',blend_train)
    np.savetxt(L1FOLD+str((100000*cv_results.mean()).astype(int))+'_'+'extra_1l_param'+str(j)+'.test.csv',blend_test)
