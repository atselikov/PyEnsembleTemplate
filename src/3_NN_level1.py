...
if __name__ == "__main__":
    ...
    parser.add_argument('alg_id', type=int, help='id')
    # parser.add_argument('xgb_bags', type=int, help='xgb_bags')
    # parser.add_argument('est', type=int, help='est')
    # parser.add_argument('max_f', type=float, help='max_f')
    # parser.add_argument('max_d', type=float, help='max_d')
    ...
    for i, (train_index, valid_index) in enumerate(SKFOLDS):
        ...
        for bag in range(BAGS):
            ...
            rs = (i+1)*1000+(bag+1)*100+j
            clf = KerasRegressor(build_fn=build_model_fn, epochs=args.epochs, batch_size=args.batch_size)
            #(n_estimators=args.est, max_features=args.max_f, max_depth =args.max_d, random_state=rs, n_jobs=-1)
    ...
    ## SAVE
    np.savetxt(L1FOLD+str((100000*cv_results.mean()).astype(int))+'_'+'nn_1l_param'+str(j)+'.train.csv',blend_train)
    np.savetxt(L1FOLD+str((100000*cv_results.mean()).astype(int))+'_'+'nn_1l_param'+str(j)+'.test.csv',blend_test)
