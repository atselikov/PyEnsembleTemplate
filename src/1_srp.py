...
if __name__ == "__main__":
    ...
    ## PROCESS
    cls = SparseRandomProjection(n_components=in_clusters, dense_output=True, random_state=RANDOM_STATE)
    cls_train = cls.fit_transform(train)
    cls_test = cls.transform(test)
    
    ## SAVE
    np.savetxt(FFOLD+'srp_'+str(in_clusters)+'_'+iinput+'.train.csv',cls_train)
    np.savetxt(FFOLD+'srp_'+str(in_clusters)+'_'+iinput+'.test.csv',cls_test)
