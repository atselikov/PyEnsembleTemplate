...
if __name__ == "__main__":
    ...
    for ifile in args.input:
        ...
        if ifile=='REM':
            break
        if k==0:
            train = pd.read_csv(FFOLD+ifile+'.train.csv', usecols=features).values
            test = pd.read_csv(FFOLD+ifile+'.test.csv', usecols=features).values
            ...
            if ifile=='ids':
                ...
                # if args.scaled == 1:
                #     cls = StandardScaler()
                #     train = np.hstack((train, cls.fit_transform(np.loadtxt(FFOLD+ifile+'.train.csv'))))
                #     test = np.hstack((test, cls.transform(np.loadtxt(FFOLD+ifile+'.test.csv'))))
                # else:
    ...
    ## SAVE
    np.savetxt(L1FOLD+str((100000*cv_results.mean()).astype(int))+'_'+'knn_f_1l_param'+str(j)+'.train.csv',blend_train)
    np.savetxt(L1FOLD+str((100000*cv_results.mean()).astype(int))+'_'+'knn_f_1l_param'+str(j)+'.test.csv',blend_test)
