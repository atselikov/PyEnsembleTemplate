...
if __name__ == "__main__":
    ...
    df = df[cat_cols]
    for col in cat_cols:
        enc = catEncoder(df[col], ncats)
        df[col] = df[col].map(enc)
    df = df.fillna(-1)
    ## SAVE
    np.savetxt(FFOLD+'2cat_catEncoded'+str(ncats)+'_'+iinput+'.train.csv',df[:len(train)].values, fmt='%10.0f')
    np.savetxt(FFOLD+'2cat_catEncoded'+str(ncats)+'_'+iinput+'.test.csv',df[len(train):].values, fmt='%10.0f')
