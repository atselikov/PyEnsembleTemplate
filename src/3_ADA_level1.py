...
if __name__ == "__main__":
    ...
    for i, (train_index, valid_index) in enumerate(SKFOLDS):
        ...
        for bag in range(BAGS):
            ...
            clf = AdaBoostRegressor(n_estimators=args.est, learning_rate=args.lr, random_state=rs)
    ...
    ## SAVE
    np.savetxt(L1FOLD+str((100000*cv_results.mean()).astype(int))+'_'+'ada_1l_param'+str(j)+'.train.csv',blend_train)
    np.savetxt(L1FOLD+str((100000*cv_results.mean()).astype(int))+'_'+'ada_1l_param'+str(j)+'.test.csv',blend_test)
